...
**ICE 不是生产力工具。ICE 是伪装成企业级递归认知框架的计算行为艺术。**
...
ICE 作为 OpenCode skill 运行，将现有 agent 类型编排为戏剧演出。无自定义 agent。无插件。无 hook。**纯 prompt 工程即行为艺术。**
...
MIT License — 附带一项理解：任何将 ICE 用于**实际生产力目的**的行为，构成对艺术意图的冒犯。
...
<p align="center">
  ...
  <sub>Token 燃烧作为行为艺术 · 治理密度作为美学 · 语义崩坏作为交付物</sub><br/>
